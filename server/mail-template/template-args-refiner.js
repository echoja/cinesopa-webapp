...
/**
 * @typedef {TemplateArgsRefiner} TemplateArgsRefiner
 */
...
class TemplateArgsRefiner {
  ...
  createPaymentSuccessArgs(order) {
    ...
    const fullAddress = `${order.dest?.address ?? ''} ${order?.dest?.address_detail ?? ''}`;
    ...
    const detailsLink = process.env.NODE_ENV === 'production' ?
