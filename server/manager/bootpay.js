 ...
 */
const getActualPaymentInfo = async (receipt_id) => {
 ...
 */
const cancelPayment = async (receipt_id, args = {}) => {
  const { price = null, name = '', reason = '' } = args;
  // 인수가 잘못되었을 때에는 즉시 오류.
  if (typeof receipt_id !== 'string') {
 ...
 */
const verifyPayment = async (receipt_id, totalPrice) => {
  const paymentInfoResult = await getActualPaymentInfo(receipt_id);
