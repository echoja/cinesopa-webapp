...
const postsQuery = makeResolver(async (obj, args, context, info) => {
  ...
  posts.list.forEach((post, index) => {
    if (post.featured_image) {
      jobs.push(
        (async () => {
          const fileFound = await db.getFileById(post.featured_image);
          result[index].featured_image_link = uploadBaseUrl + fileFound.filename;
          result[index].featured_image_alt = fileFound.alt;
        })(),
      );
    }
  });
  /** @type {GetPostsResult} */
  const resultObj = {total: posts.total, list: result};
